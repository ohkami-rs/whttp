...
/// ## example
/// ```
/// use whttp::{Header, Headers};
/// 
/// const MY_CUSTOM: &Header = &Header::def("My-Custom");
/// 
/// fn get_my_custom(headers: &Headers) -> Option<&str> {
///     headers.get(MY_CUSTOM)
/// }
/// ```
#[derive(Clone, Copy)]
pub struct Header {
...
impl Header {
    pub const fn def(name: &'static str) -> Self {
    ...
    ($( $name:ident = $bytes:literal | $lower:literal )*) => {
        pub mod standard {
            ...
            $(
                ...
                pub const $name: &Header = &Header::def(unsafe {std::str::from_utf8_unchecked($bytes)});
            )*
        }
